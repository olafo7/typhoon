...
var myFns = {
	...
	pullUpAction: function(){
		...
		var data = {'start':start, 'limit':limit,'name':name,'storeType':'returnback'};
...
$(function(){
	...
	$(document).on('touchstart','#meet_view',function(){
		...
		window.location.href = 'm.php?app=meeting&func=mgr&action=meeting&&task=loadPage&from=view&aid='+aid+'&type=returnback';
		return false;
	})
	...
	$(document).on('touchstart',"#meet_add_edit",function(){
		var aid = $(this).attr('data-aid');
		window.location.href = 'm.php?app=meeting&func=mgr&action=join&task=loadPage&from=listContent&aid='+aid+"&type=returnback";
		return false;
	})
