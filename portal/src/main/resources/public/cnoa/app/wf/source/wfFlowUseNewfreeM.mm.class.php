...
class wfFlowUseNewfreeM extends wfCommon
{
    ...
    private function _seqFlowSendNextStep( )
    {
        ...
        $uploadAttach = explode( ",", getpar( $_POST, "upload_attach" ) );
        if ( 0 < count( $uploadAttach ) )
        {
            ( );
            $fs = new fs( );
            $attach = array( );
            $attach = $fs->add( $uploadAttach, 1 );
            foreach ( $GLOBALS['_POST'] as $k => $v )
            {
                if ( strstr( $k, "wf_attach_" ) )
                {
                    $attach[] = str_replace( "wf_attach_", "", $k );
                }
            }
        }
    ...
    private function _freeFlowSendNextStep( )
    {
        ...
        $flowType = getpar( $_GET, "flowType", getpar( $_POST, "flowType", 0 ) );
        $tplSort = getpar( $_GET, "tplSort", getpar( $_POST, "tplSort", 0 ) );
        ...
        if ( 0 < count( $uploadAttach ) )
        {
            ( );
            $fs = new fs( );
            $data['attach'] = json_encode( $fs->add( $uploadAttach, 1 ) );
            foreach ( $GLOBALS['_POST'] as $k => $v )
            {
                if ( strstr( $k, "wf_attach_" ) )
                {
                    $data['attach'][] = str_replace( "wf_attach_", "", $k );
                }
            }
        }
        unset( $_POST['upload_attach'] );
