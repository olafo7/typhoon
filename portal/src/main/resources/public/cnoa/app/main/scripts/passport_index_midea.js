...
var CNOATRY;
var showTry = false;
var autoShowTry = false;

function showTryWindow(){
	try{
		CNOATRY.show();
	}catch(e){
		CNOATRY = new CNOA_tryWindow();
		CNOATRY.show();
	}
}

var CNOA_tryWindow = CNOA.Class.create();
CNOA_tryWindow.prototype = {
	init : function(){
		var _this = this;
		
		this.baseUrl = 'tools/userlist.json';
		...
		this.mainPanel = new Ext.Window({
			...
			shadow: false,
			resizable: false,
			title: lang('selectAccount'),
			closeAction: "hide",
			items: [this.deptPanel],
			buttons: [
				{
					text: lang('close'),
					handler: function() {
			...
			listeners: {
				show : function(th){
					th.getEl().alignTo(Ext.getBody(), 'br-br', [-20, -40]);
				}
			},
			tbar: [
				lang('dCtoLogin')
...
if(autoShowTry === true && showTry === true){
	CNOATRY = new CNOA_tryWindow();
	CNOATRY.show();
}
...
function CNOA_PASSPORT(){
		...
		items: [
			...
			},
			{
				...
				items: [
					{
						xtype: "displayfield",
						hidden: showTry ? false : true,
						value: "<a href='javascript:void(0);' onclick='showTryWindow()'><span class='cnoa_color_blue'>" + lang('viewTrialAccount') + "</span></a>",
						style: "margin-left:30px;"
					},
					{
						xtype: "displayfield",
						value: "<a href='javascript:void(0);' onclick='showGetPassWindow()'>" + lang('fogetPassword') + "？</a>",
						style: "margin-left:"+(showTry?10:110)+"px"
					},
					{
						xtype: "button",
						width: 60,
	...
	this.ctPanel = new Ext.Panel({
		border: false,
		bodyStyle: 'width:602px;heigth:304px;background:url(./resources/images/bg_main_midea.png);',
