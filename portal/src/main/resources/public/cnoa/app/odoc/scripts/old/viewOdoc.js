...
CNOA_odoc_send_apply_editClass.prototype = {
	init : function(){
		...
		this.baseUrl = "index.php?app=odoc&func=" + this.func + "&action="+CNOA.odoc.action + "&fromType=" + CNOA.odoc.fromType;
		this.baseURI = location.href.substr(0, location.href.lastIndexOf("/")+1);
		//alert("id:" + this.id + " - url" + this.baseUrl);
	...
	getFormHtml : function(){
		var _this = this;
		Ext.Ajax.request({  
			url: _this.baseUrl + "&task=view&act=getFormHtml&id=" + _this.id,
			method: 'POST',
			params : {id : _this.id},
			success: function(r) {
	...
	getAttachList : function(){
		...
		Ext.Ajax.request({  
			url: _this.baseUrl + "&task=view&act=getAttachList&id=" + _this.id,
			method: 'POST',
			params : {id : _this.id},
			success: function(r) {
