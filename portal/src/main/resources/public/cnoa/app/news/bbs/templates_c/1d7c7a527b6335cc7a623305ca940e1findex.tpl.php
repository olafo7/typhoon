...
<body class="background">
  <div style="width:100%; height:80px">
    <div class="search">
      <form class="form-inline" method="post" action="index.php?app=news&func=bbs&action=bbs&task=getIndex&model=search">
        ...
        今日：<span class="red"><?php echo $this->_vars['today'];?></span>　　昨日：<span class="yellow"><?php echo $this->_vars['yesterday'];?></span>　　帖子：<?php echo $this->_vars['allPosts'];?>
      </form>
    </div>
  </div>
    ...
    <div class="row">
      <div class="col-md-3">
        <div class="white" style="height:294px;">
          <div class="bottom">
            <img class="head" src="<?php echo $this->_vars['face'];?>">
            <p class="center"><?php echo $this->_vars['truename'];?></p>
          </div>
      ...
      <div class="col-md-9 white">
        <div class="row" style="height:294px;">
          ...
          <?php foreach ($this->_vars['newPost'] as $key=>$value) { ?>
            <div class="listHeight col-md-12 bottom">
            <div class="row">
              <span class="col-sm-7 center">
                <a href="index.php?app=news&func=bbs&action=bbs&task=getIndex&model=browse&fid=<?php echo $value['fid']?>&id=<?php echo $value['id']?>">  <?php echo $value['title']?>
                </a>
              </span>
              <span class="col-sm-1 center red"><?php echo $value['author']?></span>
              <span class="col-sm-2 center aaaa"><?php echo $value['deptName']?></span>
              <span class="col-sm-1 center red"><?php echo $value['reply']?></span>
              <span class="col-sm-1 center aaaa"><?php echo $value['browse']?></span>
            </div>
          </div>
          <?php } ?>
        </div>
      ...
      <div class="col-md-3 marginTop">
        <div class="white">
          ...
          <?php foreach ($this->_vars['newReply'] as $key=>$value) { ?>
          <div class="listHeight bottom">
            <span class="col-sm-9 center">
              <a href="index.php?app=news&func=bbs&action=bbs&task=getIndex&model=browse&fid=<?php echo $value['fid']?>&id=<?php echo $value['id']?>">
                <?php echo $value['title']?>
              </a>
            </span>
            <span class="col-sm-3 center red"><?php echo $value['author']?></span>
          </div>
          <?php } ?>
        </div>
      </div>
  ...
  <div class="pull-left rightDiv">
    <div class="white">
      ...
      <?php foreach ($this->_vars['weekHot'] as $key=>$value) { ?>
      <div class="listHeight2 bottom clearfix">
        <div class="a<?php echo ($key+1)?>"><?php echo ($key+1)?></div>
        <span class="bbbbbb">
          <a href="index.php?app=news&func=bbs&action=bbs&task=getIndex&model=browse&fid=<?php echo $value['fid']?>&id=<?php echo $value['id']?>">
            <?php echo $value['title']?>
          </a>
    ...
    <div class="white">
      ...
      <div class="friendLink">
        <?php foreach ($this->_vars['friendLink'] as $key=>$value) { ?>
