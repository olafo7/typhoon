...
class docReceiveApply extends model
{
    ...
    public function run( )
    {
        ...
        else if ( $task == "getNewFlowList" )
        {
            $this->_getNewFlowList( );
        }
    }
    ...
    private function _loadPage( )
    {
        ...
        if ( $from == "list" )
        {
            $tplPath = $CNOA_CONTROLLER->appPath."/tpl/default/receive/apply_list.htm";
        }
        else if ( $from == "newFlowList" )
        {
            $tplPath = $CNOA_CONTROLLER->appPath."/tpl/default/receive/apply_newFlowList.htm";
        }
    ...
    private function _getList( )
    {
        ...
        $rows = 15;
        if ( !is_array( $dispenseList ) )
        {
            $dispenseList = array( );
        }
        $ulids = array( );
        foreach ( $dispenseList as $v )
        {
            $ulids[] = $v['ulid'];
        }
        $where = "WHERE `sort`=112 AND `uid`='".$uid."' ";
    ...
    private function _getNewFlowList( )
    {
        global $CNOA_DB;
        global $CNOA_SESSION;
        $uid = $CNOA_SESSION->get( "UID" );
        $start = getpar( $_POST, "start", 0 );
        $rows = 15;
        $where = "WHERE `publish`=1 AND `sort`='112' ";
