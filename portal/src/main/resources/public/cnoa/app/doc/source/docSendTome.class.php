...
class docSendTome extends model
{
    ...
    private function _loadPage( )
    {
        ...
        if ( $from == "list" )
        {
            $tplPath = $CNOA_CONTROLLER->appPath."/tpl/default/send/tome_list.htm";
        }
        else
        {
        }
    ...
    private function _getList( )
    {
        ...
        $rows = 15;
        $where = "WHERE  `sort`=113 AND `ulid` IN (SELECT `ulid` FROM ".tname( "flow_flow_u_dispense" ).( " WHERE `to_uid`='".$uid."' AND `isread`='{$isread}') " );
