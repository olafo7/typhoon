//定义全局变量：
var CNOA_doc_receive_monit_listClass, CNOA_doc_receive_monit_list;
...
*
*/
CNOA_doc_receive_monit_listClass = CNOA.Class.create();
CNOA_doc_receive_monit_listClass.prototype = {
	init : function(){
		var _this = this;
		
		this.baseUrl = "index.php?app=doc&func=receive&action=monit";
	...
	makeOperate : function(value, cellmeta, record, rowIndex, columnIndex, color_store){
		...
		var h  = "";
			...
			h += "&nbsp;&nbsp;";
			h += "<br />";
			h += rs == 1 ? "<a href='javascript:void(0);' onclick='CNOA_doc_receive_monit_list.urge("+value+");' ext:qtip='"+lang('cuibanPerson')+"'>"+lang('reviewRecord')+"</a>" : "<span class='cnoa_color_gray'>"+lang('reviewRecord')+"</span>";
			h += "&nbsp;&nbsp;";
...
Ext.onReady(function() {
	CNOA_doc_receive_monit_list = new CNOA_doc_receive_monit_listClass();
	Ext.getCmp(CNOA.doc.receive.monit_list.parentID).add(CNOA_doc_receive_monit_list.mainPanel);
	Ext.getCmp(CNOA.doc.receive.monit_list.parentID).doLayout();
});