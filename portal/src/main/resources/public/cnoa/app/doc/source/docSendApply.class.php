...
class docSendApply extends model
{
    ...
    private function _loadPage( )
    {
        ...
        if ( $from == "list" )
        {
            $tplPath = $CNOA_CONTROLLER->appPath."/tpl/default/send/apply_list.htm";
        }
        else if ( $from == "newFlowList" )
        {
            $tplPath = $CNOA_CONTROLLER->appPath."/tpl/default/send/apply_newFlowList.htm";
        }
    ...
    private function _getList( )
    {
        ...
        $where = "WHERE `sort`=113 AND `uid`='".$uid."' ";
    ...
    private function _getNewFlowList( )
    {
        ...
        $start = getpar( $_POST, "start", 0 );
        $rows = 15;
        $where = "WHERE `publish`=1 AND `sort`='113' ";
