//定义全局变量：
var CNOA_doc_receive_apply_listClass, CNOA_doc_receive_apply_list;
...
*
*/
CNOA_doc_receive_apply_listClass = CNOA.Class.create();
CNOA_doc_receive_apply_listClass.prototype = {
	init : function(){
		var _this = this;
		
		this.baseUrl = "index.php?app=doc&func=receive&action=apply";
			...
			tbar : new Ext.Toolbar({
				...
				items: [
					...
					},
					{
						...
						allowDepress: false,
						pressed: true,
						toggleGroup: "doc_receive_apply_list",
						handler:function(){
	...
	makeOperate : function(value, cellmeta, record, rowIndex, columnIndex, color_store){
		//var rd = record.data;
		//var h  = "<a href='javascript:void(0);' onclick='CNOA_doc_receive_apply_list.viewFlow("+rd.id+");'>查看</a>";
		//return h;
		...
		var h  = "<a href='javascript:void(0);' onclick='CNOA_doc_api_flow.showFlow("+value+");'>"+lang('view')+"</a>";
			h += "&nbsp;&nbsp;";
			//状态为未发布/已退件/已撤销时
			h += (rs == '0' || rs == '3' || rs == '4') ? "<a href='javascript:void(0);' onclick='CNOA_doc_api_flow.deleteFlow("+value+");'>"+lang('del')+"</a>" : "<span class='cnoa_color_gray2'>删除</span>";
			h += "&nbsp;&nbsp;";
			//状态为未发布时
	...
	goToNewFlowList : function(){
		mainPanel.closeTab("C_M_DOC_RECEIVE_APPLY_NEWFLOWLIST");
		mainPanel.loadClass(this.baseUrl + "&task=loadPage&from=newFlowList", "C_M_DOC_RECEIVE_APPLY_NEWFLOWLIST", lang('swdj'), "icon-flow");
	},
...
Ext.onReady(function() {
	CNOA_doc_receive_apply_list = new CNOA_doc_receive_apply_listClass();
	Ext.getCmp(CNOA.doc.receive.apply_list.parentID).add(CNOA_doc_receive_apply_list.mainPanel);
	Ext.getCmp(CNOA.doc.receive.apply_list.parentID).doLayout();
});