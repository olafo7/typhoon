...
$(function(){
	...
	function getBindFlowFns(){
		...
		$.post(url,{'code':'attEgression'},function(json){
			if(json.success) {
				$('#btn_egression').attr('flowid',json.msg['flowId']);
			}
		},'json')
	}

	//外出登记记录
	function getEgressionListFns(){
		getBindFlowFns();
		$.ajax({
			type: 'get',
			dataType: 'json',
			url: 'm.php?app=att&func=person&action=egression&task=getEgressionList',
			beforeSend:function(){
				//请求数据前执行
			...
			success: function(json){
				...
				if(json.success){
					$.each(json.data,function(index,array){
						...
						if(array['address']){ //外出地点
							str += '<tr><th>外出时间:</th><td colspan="3" align="left">'+array['address']+'</td></tr>';
						}
						if(array['reason']){ //外出原因
							str += '<tr><th>外出原因:</th><td colspan="3" align="left">'+array['reason']+'</td></tr>';
						}
	...
	$(document).on('touchstart', '#btn_egression', function(){
