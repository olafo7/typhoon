...
$(function(){
	...
	function getBindFlowFns(){
		...
		$.post(url,{'code':'attEvection'},function(json){
			if(json.success){
				$('#btn_evection').attr('flowid', json.msg['flowId']);
			}
		},'json')
	}

	//出差登记记录
	function getEvectionListFun(){
		getBindFlowFns();
		$.ajax({
			type: 'get',
			dataType: 'json',
			url: 'm.php?app=att&func=person&action=evection&task=getEvectionList',
			beforeSend:function(){
				//请求数据前执行
			...
			success: function(json){
				...
				if(json.success){
					$.each(json.data,function(index,array){
						...
						str += '<tr><th>出差地点:</th><td colspan="3" align="left">'+array['address']+'</td></tr>';
						if(array['reason']){
							str += '<tr><th>出差事由:</th><td colspan="3" align="left">'+array['reason']+'</td></tr>';
						}
						if(array['days']){
							str += '<tr><th>出差总数:</th><td colspan="3" align="left">'+array['days']+'</td></tr>';
						}
	...
	$(document).on('touchstart','#btn_evection',function(){
