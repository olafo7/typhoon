...
DESKTOPAPP_CLASS.prototype = {
	init : function(portalsID){
		...
		var me = this,
			desktopId = "CNOA_MAIN_DESKTOP_USER_MYCUSTOMERS";
	...
	chartPanel: function(){
		...
		var s_time = new Ext.TimeInterval({showMode: 'year'}),
			searchParams = {
        ...
        var chart = new Ext.Panel({
        	border: false,
	        html: "<div style='width: 100%; height: 490px;' id='CNOA_ID_MY_CUSTOMERS"+rand_ID+"'>",
				...
				items:[
					...
					s_time,
					{
						...
						handler: function(){
							...
							$("#CNOA_ID_MY_CUSTOMERS"+rand_ID).html();
							Ext.Ajax.request({
								url: _this.baseUrl + "&task=myCustomers",
								...
								success: function(h) {
									...
									funnel.setJSONData({
			        					...
			        					canvasBgAlpha: 0,//透明度
										baseFontSize: 14,//字体大小
										formatNumberScale: 0,//单位自动加上K/M
										decimalPrecision: 2,//小数位数
										data: rp.data
									});
									funnel.render("CNOA_ID_MY_CUSTOMERS"+rand_ID);
								}
							})
						}
					}
