$(function(){
	...
	$(document).on('touchstart','#add_edit_summary',function(){//跳转的瞬间隐藏掉底部计划
		$('#jingle_popup').css('display','none');
	    $('#jingle_popup_mask').hide();
	    return false;
	});
	$(document).on('touchstart','#add_edit_additional',function(){//跳转的瞬间隐藏掉底部计划
		$('#jingle_popup').css('display','none');
	    $('#jingle_popup_mask').hide();
	    return false;
	});
	...
	//对添加修改总结按钮附加参数和跳转地址
	$(document).on('touchstart','#add_edit_summary',function(){
		window.location.href = 'm.php?app=user&func=diary&action=index&task=loadPageAddSummary&date=' + GetQueryString('date');
		return false;
	});

	$(document).on('touchstart','#add_edit_additional',function(){
		window.location.href = 'm.php?app=user&func=diary&action=index&task=addEditAdditional&date=' + GetQueryString('date');
		return false;
	});
