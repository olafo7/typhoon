...
myFns = {
	...
	getReportList: function(type, dataType, data, emptyContainer, emptyTbody, mask) {
		$.ajax({
			url: 'm.php?app=report&func=form&action=view&modul=wf&task=getJsonData',
			type: type,
			dataType: dataType,
			data: data,
			beforeSend: function() {
				...
				if (json.success) {
					if (emptyContainer) {
						...
						var theadHtml  = '<table class="table table-bordered"><thead><tr>';
					 	 	 theadHtml += '<th>No</th><th>操作</th><th>报表名称</th><th>开始时间</th><th>结束时间</th><th>设计人</th></tr></thead><tbody></tbody></table>';
					...
					if (emptyTbody) {
			     	...
			     	for (var i = 0; i < dataLen; i++) {
			     		var tbodyHtml   = '<tr><td>'+(bTrLen<1 ? (i+1) : (bTrLen+i+1))+'</td><td>';
			     			 tbodyHtml += '<button type="button" class="btn btn-primary btn-xs vReport" tid='+data[i].tid+'>查看</button>';
					     	 if (data[i].history == 1) {
					     	 	tbodyHtml += '<button type="button" class="btn btn-success btn-xs vhistoryReport" condition='+data[i].condition+'>历史报表</button>'; 
					     	 };
			     			 tbodyHtml += '</td>';
			     			 tbodyHtml += '<td>'+data[i].name+'</td><td>'+myFns.convertEmptyData(data[i].stime)+'</td>';  
					     	 tbodyHtml += '<td>'+myFns.convertEmptyData(data[i].etime)+'</td><td>'+data[i].postname+'</td>';
					     	 tbodyHtml += '</tr>';
...
$(function(){
	...
	//跳转到用户报表查看
	$('#userView').on('touchstart',function(){
	...
	$(document).on("click", "button.hvReport, button.vReport", function(e) {
		var el = $(e.target), hid = $(this).attr('hid'), tid = $(this).attr('tid');
		if (el.is('.vReport')) {
			window.location.href = 'm.php?app=report&func=form&action=view&modul=wf&task=loadPage&from=detail&tid='+tid+'';
