...
myFns = {
  ...
  getTempleDataList: function(isEmpty, params) {
    ...
    var tid = myFns.getUriString("tid"), hid = myFns.getUriString("hid") != null ? myFns.getUriString("hid") : 0;
    if (!params) {
      var data = {"start": start, "limit": limit};
    } else {
      var data = {"start": start, "limit": limit, "kong1": params.fieldsName, "value1": params.keyword, "condition1": params.condition, "search": params.search};
    }
    $.ajax({
      url: 'm.php?app=report&func=form&action=view&modul=wf&tid='+tid+'&hid='+hid+'&task=getTempleJsonData',
      type: 'POST',
      dataType: 'json',
      data: data,
      beforeSend: function() {
      ...
      success: function(json) {
        ...
        if (json.success) {
          ...
          for (var i = 0; i < dataLen; i++) {
            tdHtml += '<tr uFlowId="'+data[i].uFlowId+'">';
...
jQuery.reportFiles = {
  ...
  getAttachList: function() {
    $.ajax({
      url: 'm.php?app=report&func=form&action=view&modul=wf&tid='+this.param.tid+'&hid='+this.param.hid+'&task=getAttachList&uFlowId='+this.param.uFlowId+'&type='+this.param.type+'',
      type: 'GET',
      dataType: 'json',
      beforeSend: function() {
        showLoading();
...
$(function() {
  ...
  function loadColumn(callbackFunc) {
    var tid = myFns.getUriString("tid"), hid = myFns.getUriString("hid") != null ? myFns.getUriString("hid") : 0;
    $.ajax({
      url: 'm.php?app=report&func=form&action=view&modul=wf&tid='+tid+'&hid='+hid+'&task=loadColumn',
      type: 'GET',
      dataType: 'json',
      beforeSend: function() {
        showLoading();
