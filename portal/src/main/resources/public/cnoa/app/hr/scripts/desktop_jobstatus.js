...
DESKTOPAPP_CLASS.prototype = {
	init : function(portalsID){
		...
		var me = this,
			desktopId = "CNOA_MAIN_DESKTOP_JOBSTATUS";
	...
	chartPanel: function(){
		...
		var chart = new Ext.Panel({
			border: false,
	        html: "<div style='width: 100%; height: 490px;' id='CNOA_JOB_STATUS_CHART"+rand_ID+"'>",
	        listeners: {
	        	afterrender: function(){
	        		Ext.Ajax.request({
	        			url: _this.baseUrl + "&task=jobStatus",
	        			success : function(response){
	        				var rp = Ext.decode(response.responseText);
	        				var  mychart = new FusionCharts("scripts/fusioncharts/Column3D.swf", "myChartId"+rand_ID, '100%', '93%', "0", "1");   
	        				mychart.setJSONData({
	        					"chart": {
	        						"caption": "人事在职状态(图)"
	        					},
	        				...
	        				mychart.render("CNOA_JOB_STATUS_CHART"+rand_ID);
	        			}
	        		})
	        	}
	        }
